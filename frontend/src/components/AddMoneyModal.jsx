...
const AddMoneyModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleAddMoney = async (e) => {
    ...
    try {
      ...
      await API.post('/transactions/add-money', { amount: numAmount });
      
      // Refresh the global user state (balance)
      ...
      <div className="relative w-full max-w-md bg-slate-900 border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl">
        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-gradient-to-r from-blue-600/10 to-transparent">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-500/20 rounded-lg text-blue-400"><Wallet size={20} /></div>
        ...
        <form onSubmit={handleAddMoney} className="p-8 space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="0.00"
                className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 pl-12 pr-6 text-2xl font-bold text-white focus:ring-2 focus:ring-blue-500 outline-none transition-all"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {quickAmounts.map((amt) => (
              <button key={amt} type="button" onClick={() => setAmount(amt.toString())}
                className="py-2 bg-white/5 border border-white/10 rounded-xl text-sm font-semibold text-slate-300 hover:bg-blue-600 hover:text-white transition-all">
                +{amt}
              </button>
            ))}
          </div>
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 transition-all">
            {loading ? <Loader2 className="animate-spin" size={20} /> : <>Confirm Top-up <ArrowUpRight size={20} /></>}
          </button>
        </form>
      </div>
    </div>
